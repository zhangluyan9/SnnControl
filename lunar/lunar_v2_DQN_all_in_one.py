...
CNN = True
SNN = False
Quantized_activation_level = 4
Integer_Weight = False
HDC = False
MPC = False
add_noise = [True,0.08,3]
...
def get_mem_p(out,fc):
                ...
                for i in range(out.shape[0]):
                    out_[i] = out[i].mT
                out_s = fc(out_[0][0].reshape(1,64))
                #V
...
class Net(nn.Module):
    ...
    def forward(self, state):
        ...
        #if add_noise[0]:
        #    noise = add_noise[1]*torch.tensor(np.random.randn(state.shape[0],state.shape[1])).type(torch.FloatTensor)
        #    state+=noise
        ...
        if SNN:
            ...
            out_s = self.fc3(out_[0][0].reshape(1,64))
            #V
            for i in range(Quantized_activation_level-1):
...
class DQN:
    ...
    def load_model(self):
        ...
        if SNN:
            ...
            self.eval_net.load_state_dict(torch.load("checkpoint_64_1_cq_q_level4.pth"), strict=False)
        if CNN:
            self.eval_net.load_state_dict(torch.load("checkpoint_64_1_1.pth"), strict=False)
...
if __name__ == "__main__":
    ...
    if HDC:
        ...
        #checkpoint_64_1_1_0430
        model_.load_state_dict(torch.load("checkpoint_64_1_cq_q_level4.pth"), strict=True)
        #model.load_state_dict(torch.load("checkpoint_64_1_cq_q_level4.pth"), strict=True)
        represent_ = torch.zeros((4,64*Quantized_activation_level))
        ...
        print(represent_)
        represent_ = torch.where(represent_<=1894.558, my_zeros, represent_)
        represent_ = torch.where(represent_>=1894.558 , my_ones, represent_)
        print(represent_)
            ...
            for i_episode in range(t_):
                ...
                for t in range(1000):
                    ...
                    if done:
                        break 
                scores+=score
            #print(scores/t_,additions_s_l1/(t_*1000),additions_s_l2/(t_*1000))
            print(scores/t_)
